...
#include "berry_pkg/BerryGetPosition.h"
...
bool BerryGetPresentPositionCallback(
  berry_pkg::SyncGetPosition::Request &req,
  berry_pkg::SyncGetPosition::Response &res)
{
  uint8_t dxl_error = 0;
  ...
  std::cout <<dxl_comm_result<< std::endl;
...
void BerrySetPositionCallback(const berry_pkg::BerrySetPosition::ConstPtr & msg)
{
  uint8_t dxl_error = 0;
  int dxl_comm_result = COMM_TX_FAIL;
  int dxl_addparam_result = false;
...
int main(int argc, char ** argv)
{
  ...
  ros::init(argc, argv, "Berry_read_write_node");
  ros::NodeHandle nh;
  ros::ServiceServer Berry_get_position_srv = nh.advertiseService("/Berry_get_position", BerryGetPresentPositionCallback);
  ros::Subscriber Berry_set_position_sub = nh.subscribe("/Berry_set_position", 10, BerrySetPositionCallback);
  ros::spin();
